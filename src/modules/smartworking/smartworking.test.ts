...
// ──────────────────────────────────────────────
// 2. generateAllPermutations — NUOVO ALGORITMO BASE-3
// ──────────────────────────────────────────────
describe('generateAllPermutations', () => {
  ...
  describe('0 giorni liberi (tutti fissati)', () => {
    ...
    it('2 SW + 1 half + 2 Office → 1 permutazione, valida (2.5 SW, 2.5 Office)', () => {
      const dayStates = ['sw', 'sw', 'half', 'office', 'office'] as const
      const result = generateAllPermutations(dayStates as any, 2.5, 2.5)
      expect(result).toHaveLength(1)
      expect(result[0].valid).toBe(true)
      expect(result[0].totalSW).toBe(2.5)
      expect(result[0].totalOffice).toBe(2.5)
    })
  })
  ...
  // ── 2h. Giorni pre-fissati con half ──
  describe('giorni pre-fissati con half', () => {
    it('1 half fisso + 4 liberi, target 3.0 SW / 2.0 Office', () => {
      const dayStates = ['half', 'free', 'free', 'free', 'free'] as const
      const result = generateAllPermutations(dayStates as any, 3.0, 2.0)
      // fixed: 0.5 SW + 0.5 Office. Need: 2.5 SW + 1.5 Office from 4 free days
      const valid = result.filter(p => p.valid)
      expect(valid.length).toBeGreaterThan(0)
      for (const p of valid) {
        expect(p.totalSW).toBe(3.0)
        expect(p.totalOffice).toBe(2.0)
        expect(p.week[0]).toBe('half') // giorno 0 resta half
      }
    })

    it('2 half fissi + 3 liberi, target 2.5 SW / 1.5 Office (4gg lavorati)', () => {
      const dayStates = ['half', 'half', 'free', 'free', 'absent'] as const
      const result = generateAllPermutations(dayStates as any, 2.5, 1.5)
      // fixed: 1.0 SW + 1.0 Office. Need: 1.5 SW + 0.5 Office from 3 free days
      const valid = result.filter(p => p.valid)
      expect(valid.length).toBeGreaterThan(0)
      for (const p of valid) {
        expect(p.totalSW).toBe(2.5)
  ...
  // ── 2j. Proprietà: somma SW+Office = giorni lavorati ──
  describe('proprietà: totalSW + totalOffice = giorni lavorati', () => {
    const cases = [
      ...
      { dayStates: ['half','free','free','free','free'] as const, worked: 5, swT: 3.0, offT: 2.0 },
    ]
  ...
  // ── 2k. Nessuna permutazione valida se target impossibile ──
